    ...
    request.requestBody({
     ...
     if(response.result.status === "COMPLETED"){
        ...
        if(update.modifiedCount>0){
            console.log(uid,id)
           const result= await Transaction.updateOne({userId:new mongoose.Types.ObjectId(uid),orderId:new mongoose.Types.ObjectId(id)},{$set:{paymentRefundedAt:new Date()}})
           if(result.modifiedCount>0){
...
exports.getSearchItem=async(req,res)=>{
    try{
        ...
        const response=await OrderItem.find({name:{ $regex:search, $options: "i" }}).populate({path:"orderid",select:" _id orderstatus paymentstatus"})
